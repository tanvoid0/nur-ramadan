...
import { View, Habit, QuranProgress, User, HabitType, TaskCategory, PrayerNotificationSettings, ClockStyle, Theme } from './types';
...
const App: React.FC = () => {
  ...
  if (activeView === 'auth' && !user) return <AuthView onLogin={async (u) => { 
    localStorage.setItem('nur_active_user', JSON.stringify(u));
    await db.saveUser(u); setUser(u); setActiveView('home');
  }} />;
    ...
    <ErrorBoundary>
      <div className="flex min-h-screen bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100 transition-colors duration-300">
        <aside className="hidden md:flex w-64 flex-col bg-slate-900 text-white fixed h-full z-50">
          <div className="p-6">
            ...
            <nav className="space-y-1">
              <SidebarButton active={activeView === 'home'} icon={<Home />} label="Home" onClick={() => setActiveView('home')} />
              <SidebarButton active={activeView === 'habits'} icon={<CheckSquare />} label="Routine" onClick={() => setActiveView('habits')} />
              <SidebarButton active={activeView === 'quran'} icon={<BookOpen />} label="Quran" onClick={() => setActiveView('quran')} />
              <SidebarButton active={activeView === 'kitchen'} icon={<Utensils />} label="Kitchen" onClick={() => setActiveView('kitchen')} />
              <SidebarButton active={activeView === 'settings'} icon={<SettingsIcon />} label="Settings" onClick={() => setActiveView('settings')} />
            </nav>
          </div>
          <div className="mt-auto p-6 border-t border-white/5">
            <div className="flex items-center gap-3 p-3 rounded-2xl bg-white/5">
              ...
              <button onClick={logout} aria-label="Logout" className="p-2 text-white/40 hover:text-white"><LogOut className="w-4 h-4" /></button>
            </div>
          </div>
        </aside>
        ...
        <div className="flex-1 md:ml-64 relative flex flex-col">
            ...
            <div className="max-w-4xl mx-auto w-full relative z-10">
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex gap-2 md:hidden">
                  <button onClick={() => setActiveView('settings')} aria-label="Settings" className="bg-white/10 p-2.5 rounded-2xl border border-white/20"><SettingsIcon className="w-5 h-5" /></button>
                  <button onClick={logout} aria-label="Logout" className="bg-white/10 p-2.5 rounded-2xl border border-white/20"><LogOut className="w-5 h-5" /></button>
                </div>
              </div>
          ...
          <main className="flex-1 p-6 pb-32 md:pb-12 overflow-y-auto">
            <div className="max-w-4xl mx-auto w-full">
              {activeView === 'home' && <Dashboard habits={habits} quran={quran} prayerData={prayerData} locationName={locationName} currentDate={currentDate} onDateChange={changeDate} onNavigate={setActiveView} />}
          ...
          <nav className="fixed bottom-0 left-0 right-0 md:hidden bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-t border-slate-200 dark:border-slate-800 p-4 flex justify-around items-center z-50">
            <NavButton active={activeView === 'home'} icon={<Home />} label="Home" onClick={() => setActiveView('home')} />
            <NavButton active={activeView === 'habits'} icon={<CheckSquare />} label="Routine" onClick={() => setActiveView('habits')} />
            <NavButton active={activeView === 'quran'} icon={<BookOpen />} label="Quran" onClick={() => setActiveView('quran')} />
            <NavButton active={activeView === 'kitchen'} icon={<Utensils />} label="Kitchen" onClick={() => setActiveView('kitchen')} />
          </nav>
        </div>
      </div>
    </ErrorBoundary>
...
// Use React.ReactElement to ensure icon is a clonable element and satisfy TypeScript's check on props
const SidebarButton: React.FC<{active: boolean; icon: React.ReactElement; label: string; onClick: () => void}> = ({ active, icon, label, onClick }) => (
  <button onClick={onClick} aria-label={label} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${active ? 'bg-primary-600 text-white shadow-lg shadow-primary-900/40' : 'text-white/50 hover:bg-white/5 hover:text-white'}`}>
...
// Use React.ReactElement to ensure icon is a clonable element and satisfy TypeScript's check on props
const NavButton: React.FC<{active: boolean; icon: React.ReactElement; label: string; onClick: () => void}> = ({ active, icon, label, onClick }) => (
  <button onClick={onClick} aria-label={label} className={`flex flex-col items-center gap-1.5 transition-all ${active ? 'text-primary-700 dark:text-primary-400 scale-110' : 'text-slate-400'}`}>
